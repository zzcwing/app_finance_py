...
class cnumber:
    ...
    def __csplit(self,cdata): #拆分函数，将整数字符串拆分成[亿，万，仟]的list
        g=len(cdata)%4
        csdata=[]
        lx=len(cdata)-1
        if g>0:
    ...
    def __cschange(self,cki): #对[亿，万，仟]的list中每个字符串分组进行大写化再合并
        lenki=len(cki)
        i=0
        lk=lenki
        chk=u''
    ...
    def cwchange(self,data):
        ...
        i=0
        chk=u''
        cski=self.__csplit(cki) #分解字符数组[亿，万，仟]三组List:['0000','0000','0000']
        ikl=len(cski) #获取拆分后的List长度
        #大写合并
        for i in range(ikl):
            if self.__cschange(cski[i])=='': #有可能一个字符串全是0的情况
                chk=chk+self.__cschange(cski[i]) #此时不需要将数字标识符引入
            else:
                chk=chk+self.__cschange(cski[i])+self.xdict[ikl-i] #合并：前字符串大写+当前字符串大写+标识符
        #处理小数部分
        lenkj=len(ckj)
        if lenkj==1: #若小数只有1位
            if int(ckj[0])==0: 
...
if __name__=='__main__':
    ...
    print pt.cwchange('600000001000.80')